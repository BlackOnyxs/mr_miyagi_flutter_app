...
class FoodCardModelView extends BaseModel with Validators{
  ...
  void addFood( FoodModel food ){
    ...
    if ( !_cartService.currentState ) {
      ...
      _newOrder.localFoods.add(food);
      _subTotal += double.parse(food.price);
    ...
    _cartService.currentOrder.localFoods.add(food);
    _subTotal = double.parse(_cartService.currentOrder.subTotalPrice);
    _cartService.currentOrder.subTotalPrice = (_subTotal + double.parse(food.price)).toStringAsFixed(2);
    _cartService.currentOrder.totalPrice = (_subTotal + double.parse(food.price)).toStringAsFixed(2);
  }
